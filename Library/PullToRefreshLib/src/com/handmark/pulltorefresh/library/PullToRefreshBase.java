...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	private OnRefreshPositionListener onRefreshPositionListener;
	public void setOnRefreshPositionListener(OnRefreshPositionListener listener){
		this.onRefreshPositionListener=listener;
	}
	public interface OnRefreshPositionListener{
		void onRefreshPosition(float positionY);
	}
	@Override
			...
			case MotionEvent.ACTION_UP: {
				if (mIsBeingDragged) {
					...
					if (mState == State.RELEASE_TO_REFRESH
						...
						if (onRefreshPositionListener!=null){
							onRefreshPositionListener.onRefreshPosition(Math.round(Math.min(mInitialMotionY - mLastMotionY, 0) / FRICTION));
						}
	...
	//edit by wind (protected to public)
	...
	private OnHeaderScrollChangeListener onHeaderScrollChangeListener;
	public void setOnHeaderScrollChangeListener(OnHeaderScrollChangeListener onHeaderScrollChangeListener){
		this.onHeaderScrollChangeListener=onHeaderScrollChangeListener;
	}
	//add by wind
	public interface OnHeaderScrollChangeListener{
		void onHeaderScrollChange(int scrollY);
	}
	/**
	...
	public final void setHeaderScroll(int value) {
		...
		if (onHeaderScrollChangeListener!=null){
			onHeaderScrollChangeListener.onHeaderScrollChange(value);
		}
